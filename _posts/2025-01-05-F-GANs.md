...
At iteration $t-1$, we begin by finding a suboptimal discriminator $g_f(V_\omega(x)_{t-1})$ for the current
density output from the generator $Q_\theta_{t-2}$ which is assumed fixed in this iteration.
We then update the density $Q_\theta_{t-2} \rightarrow Q_\theta_t$ fixing the updated discriminator
$g_f(V_\omega(x)_{t-1})$ to enhance accuracy.
Repeating this cycle ultimately guides us to the desired solution.