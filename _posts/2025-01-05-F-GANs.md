...
$$
\begin{align}
D_f(P \| Q) = \int_(\textchi)  f\left( \frac{dP}{dQ} (x) \right) dQ(x) \\
= \int_(\textchi) \sup_{y \in dom(f^*)} \left( y\frac{dP}{dQ} (x) - f^*(y) \right) dQ(x) \\
\ge \int_(\textchi) \left( g(x)\frac{dP}{dQ} (x) - f^*(g(x)) \right) dQ(x) \\
= \int_(\textchi) g(x)\frac{dP}{dQ} (x) dQ(x) - \int_(\textchi) f^*(g(x)) (x) dQ(x) \\
