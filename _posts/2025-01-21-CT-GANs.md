...
The learned Gaussian mixture is $\mathbb{P}_{C_i}(c_{i,j}) = \sum_{k=1}^{n} \mu_k \mathcal{N}(c_{i,j}; \eta_k, \phi_k)$.

- 2. For each value $c_{i,j}$ in $C_i$ calculate the probability of $c_{i,j}$ coming from each mode $\rho_{k,i,j} = \mu_k \mathcal{N}(c_{i,j}; \eta_k, \phi_k)$.
