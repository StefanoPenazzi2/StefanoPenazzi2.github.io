...
The objective is to resample efficiently so that all categories $k_{D_i}^j$ of discrete attributes
...
$$
\mathbf{X} = Y \mid k_{D_i}^j \sim Q_\theta 
$$
...
$$
\mathbf{P}(r) = \sum_{k \in k_{D_i}} Q_\theta(X=x) \mathbf{P}(k_{D_i}=k) 
$$
...
Let $\mathbf{m}_i = [m_{D_i}]$ be the mask vector associated with the one-hot vector representation of $D_i$.
...
$$
\mathbf{c} = \mathbf{m}_{D_1} \oplus \ldots \oplus \mathbf{m}_{D_{N_d}}. 
$$
...
$$ 
\min_{\theta} \max_{\omega} F(\theta, \omega) = \min_{\theta} \max_{\omega} \left( \mathbb{E}_{x \sim P} \left[g_f(V_\omega(y | m_i^k))\right] + \mathbb{E}_{x \sim Q_\theta} \left[\log(-g_f(V_\omega(y | m_i^k)))\right] + 1 + \lambda \cdot H(m_i^k, \hat{m_i^k}) \right) 
$$
