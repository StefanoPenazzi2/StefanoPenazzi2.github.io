...
Class imbalance in categorical columns $D_i$ poses challenges for standard GAN generator training.
...
$$
\mathbf{x} \sim Q_\theta(x \setminus D_{i^*} \mid D_{i^*} = k^*) 
$$
...
$$
\mathcal{P}(r) = \sum_{k \in D_{i^*}} Q_\theta(x \setminus D_{i^*} \mid D_{i^*} = k^*) \mathcal{P}(D_{i^*} = k) 
$$
